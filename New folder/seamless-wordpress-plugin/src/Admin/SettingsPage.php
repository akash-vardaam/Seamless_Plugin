...
?>
	...
	?>
		...
		<?php if (!$this->auth->is_authenticated()): ?>
			<div class="seamless-api-notice" style="padding: 20px; background: #fff; border: 1px solid #ccd0d4; margin-top: 10px;">
		...
		<script>
			jQuery(document).ready(function($) {
				...
				if (urlParams.get('settings-updated') === 'true' || urlParams.get('settings-updated') === '1') {
					...
					// Delay cleanup slightly to let user see it, but handle referer immediately if possible? 
					// Actually, for settings-updated, WP adds it on redirect, so we just need to clean it from URL bar
					// But we DO want to clean it from referer so next save doesn't double it (though WP handles that usually)
					setTimeout(function() {
					...
					$.post(ajaxurl, {
						...
						if (response.success) {
							var redirectUrl = window.location.href.includes('view=settings') ?
								'?page=seamless&tab=authentication&auth_success=1' :
								'?page=seamless&tab=authentication&auth_success=1';
