...
function seamless_register_react_events_api_routes()
{
    ...
    register_rest_route('seamless/v1', '/categories', array(
        'methods' => 'GET',
        'callback' => 'seamless_api_get_categories',
        'permission_callback' => '__return_true',
    ));
...
/**
 * API Handler: Get Categories List
 *
 * Fetches categories from the external API and returns them to React app
 *
 * @param WP_REST_Request $request The REST request object
 * @return WP_REST_Response
 * @since 1.0.0
 */
function seamless_api_get_categories($request)
{
    $api_endpoint = 'https://mafp.seamlessams.com/api';
...
function seamless_api_get_single_event($request)
{
    ...
    // Make request to third-party API
    $url = $api_endpoint . '/events/' . sanitize_text_field($id) . '/';
